...
Backbone.Roadrunner = function(ref) {
  this._fbref = ref;
  this._children = [];
...
_.extend(Backbone.Roadrunner.prototype, {

  _childAdded: function(childSnap, prevChild) {
    var model = childSnap.val();
    console.log("_childAdded",model);
  ...
  _childChanged: function(childSnap, prevChild) {
    var model = childSnap.val();
    console.log("_childChanged",model);
    model.id = childSnap.name();
    var item = _.find(this._children, function(child) {
      return child.id == model.id
    });
  ...
  _childRemoved: function(oldChildSnap) {
    ...
    console.log("_childRemoved",model);
    this._children = _.reject(this._children, function(child) {
      return child.id == model.id
    });
  },
...
// Override 'Backbone.sync' to default to Roadrunner sync.
...
Backbone.Roadrunner.Collection = Backbone.Collection.extend({
  sync: function() {
    this._log("Sync called on a Roadrunner collection, ignoring.");
  },

  fetch: function() {
    this._log("Fetch called on a Roadrunner collection, ignoring.");
  },
  ...
  _childAdded: function(snap) {
    this._log("_childAdded called with " + snap.val());
    var model = snap.val();
  ...
  _childChanged: function(snap) {
    this._log("_childChanged called with " + snap.val());
    var model = snap.val();
    ...
    if (!item) {
      ...
      throw new Error("Could not find model with ID " + model.id,snap);
    }
    item.set(model);
  },

  _childRemoved: function(snap) {
    this._log("_childRemoved called with " + snap.val());
