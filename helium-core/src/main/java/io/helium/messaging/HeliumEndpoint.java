...
import io.helium.event.changelog.ChangeLogEvent;
import io.helium.event.changelog.ChildAddedLogEvent;
import io.helium.event.changelog.ChildChangedLogEvent;
import io.helium.event.changelog.ChildRemovedLogEvent;
...
public class HeliumEndpoint implements HeliumEventDistributor, HeliumOutboundSocket {
	...
	private HeliumOutboundSocket	heliumSocket;
	...
	public HeliumEndpoint(String basePath, Node auth, HeliumOutboundSocket heliumSocket,
			Persistence persistence, Authorization authorization, Helium helium) {
		this.heliumSocket = heliumSocket;
	...
	public void fireChildAdded(String name, Path path, Path parent, Object node, boolean hasChildren,
		...
		if (authorization.isAuthorized(HeliumOperation.READ, auth, persistence.getRoot(), path,
			...
			heliumSocket.send(broadcast.toString());
		}
	}
