...
import de.skiptag.roadrunner.core.RoadrunnerSnapshot;
...
public class RoadrunnerService implements DataListener {
	...
	public RoadrunnerService(AuthorizationService authorizationService, DataService dataService, String contextName,
			String path) {
	...
	public void child_moved(JSONObject childSnapshot, String prevChildName, boolean hasChildren, long numChildren) {
		try {
			RoadrunnerSnapshot roadrunnerSnapshot = new RoadrunnerSnapshot(authorizationService, dataService,
					contextName, path, childSnapshot, null, numChildren, prevChildName, hasChildren, 0);
	...
	public RoadrunnerService parent() {
		return new RoadrunnerService(authorizationService, dataService, contextName, dataService.getParent(path));
	}
	...
	public RoadrunnerService push(JSONObject data) {
		try {
			...
			return new RoadrunnerService(authorizationService, dataService, contextName, (path.endsWith("/") ? path
					: path + "/") + name);
		} catch (Exception exp) {
	...
	public void query(String expression, final SnapshotHandler handler) {
		dataService.query(expression, new QueryCallback() {
			@Override
			public void change(String path, JSONObject value, String parentPath, long numChildren, String name,
					boolean hasChildren, int priority) {
				RoadrunnerSnapshot snap = new RoadrunnerSnapshot(authorizationService, dataService, name, path, value,
	...
	public RoadrunnerService root() {
		return new RoadrunnerService(authorizationService, dataService, contextName, "/");
	...
	public RoadrunnerService push(org.mozilla.javascript.NativeObject obj) throws JSONException {
		JSONObject data = toJSONObject(obj);
		return push(data);
	}

	public RoadrunnerService update(org.mozilla.javascript.NativeObject data) throws JSONException {
		return update(toJSONObject(data));
	}

	public RoadrunnerService set(org.mozilla.javascript.NativeObject data) throws JSONException {
		return set(toJSONObject(data));
	}

	public static JSONObject toJSONObject(NativeObject data) throws JSONException {
