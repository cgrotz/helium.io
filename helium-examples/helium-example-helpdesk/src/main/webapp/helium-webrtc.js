function HeliumWebRTC(ref, sourceVideoSelector, remoteVideoSelector) {
	if (typeof ref == "string") {
    this.helium = new Helium(ref);
  } else {
    this.helium = ref;
  }
	...
	this.disconnect = function() {
			...
			self.helium.send(JSON.stringify({
				type : "bye"
			}));
			stop();
			peerConn.close();
	...
	this.setLocalAndSendMessage = function(sessionDescription) {
		...
		self.helium.send(sessionDescription);
	};
	...
	this.registerHelium = function() {
		self.helium.on('event', function(snapshot) {
