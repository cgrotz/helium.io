...
Backbone.Helium = function(ref) {
  this._fbref = ref;
  this._children = [];
  if (typeof ref == "string") {
    this._fbref = new Helium(ref);
  }
  _.bindAll(this);
...
_.extend(Backbone.Helium.prototype, {
...
Backbone.Helium.sync = function(method, model, options, error) {
  var store = model.helium || model.collection.helium;
...
// Override 'Backbone.sync' to default to Helium sync.
// the original 'Backbone.sync' is still available in 'Backbone.oldSync'
Backbone.sync = function(method, model, options, error) {
  var syncMethod = Backbone.oldSync;
  if (model.helium || (model.collection && model.collection.helium)) {
    syncMethod = Backbone.Helium.sync;
  }
  return syncMethod.apply(this, [method, model, options, error]);
};

// Custom Helium Collection.
Backbone.Helium.Collection = Backbone.Collection.extend({
  sync: function() {
    this._log("Sync called on a Helium collection, ignoring.");
  },

  fetch: function() {
    this._log("Fetch called on a Helium collection, ignoring.");
  },
  ...
  constructor: function(models, options) {
    ...
    if (options && options.helium) {
      this.helium = options.helium;
    }
    switch (typeof this.helium) {
      case "object": break;
      case "string": this.helium = new Helium(this.helium); break;
      case "function": this.helium = this.helium(); break;
      default: throw new Error("Invalid helium reference created");
    }

    // Add handlers for remote events.
    this.helium.on("child_added", _.bind(this._childAdded, this));
    this.helium.on("child_moved", _.bind(this._childMoved, this));
    this.helium.on("child_changed", _.bind(this._childChanged, this));
    this.helium.on("child_removed", _.bind(this._childRemoved, this));

    // Add handlers for all models in this collection, and any future ones
    // that may be added.
    function _updateModel(model, options) {
      this.helium.ref().child(model.id).update(model.toJSON());
    }
  ...
  add: function(models, options) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      var model = parsed[i];
      this.helium.ref().child(model.id).set(model, _.bind(options.success, model));
    }
  },
  ...
  remove: function(models, options) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      var model = parsed[i];
      this.helium.ref().child(model.id).set(null, _.bind(options.success, model));
    }
  },
  ...
  _parseModels: function(models) {
    ...
    for (var i = 0; i < models.length; i++) {
      ...
      if (!model.id) {
        model.id = this.helium.ref().push().name();
      }
      ret.push(model);
    }
    return ret;
...
// Custom Helium Model.
Backbone.Helium.Model = Backbone.Model.extend({
  save: function() {
    this._log("Save called on a Helium model, ignoring.");
  },

  destroy: function(options) {
    // TODO: Fix naive success callback. Add error callback.
    this.helium.ref().set(null, this._log);
  ...
  constructor: function(model, options) {
    ...
    // Add handlers for remote events.
    this.helium.on("value", this._modelChanged.bind(this));
  ...
  _updateModel: function(model, options) {
    ...
    // so Helium properly deletes them.
    var modelObj = model.toJSON();
    _.each(model.changed, function(value, key) {
      if (typeof value === "undefined" || value === null)
        modelObj[key] = null;
    });
    this.helium.ref().update(modelObj, this._log);
  },
