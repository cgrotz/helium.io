...
public class InMemoryPersistence implements Persistence {
	...
	for (String childNodeKey : node.keys()) {
		...
		handler.fireChildAdded(childNodeKey, pathForReturn, parentName, object, hasChildren, numChildren, null, indexOf);
	    }
	}
    }
	...
	if (node.has(path.getLastElement())) {
	    ...
	    handler.fireValue(childNodeKey, path.toString(), path.getParent()
		    .toString(), object, "", node.indexOf(childNodeKey));
	} else {
	    handler.fireValue(childNodeKey, path.toString(), path.getParent()
		    .toString(), "", "", node.indexOf(childNodeKey));
	}
    }
