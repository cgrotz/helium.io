...
public class ModeShapeDataService implements DataService, EventListener {
	...
	try {
		...
		if (!event.getPath().startsWith("/queries")) {
			...
			fireChildAdded(node.getName(), node.getPath(), parentName, transformToJSON(node), getPrevChildName(node), node.hasNodes(), node.getNodes()
				.getSize());
			...
			fireChildChanged(node.getName(), node.getPath(), parentName, transformToJSON(node), getPrevChildName(node), node.hasNodes(), node.getNodes()
				.getSize());
