...
// Define the `roadrunner` module under which all AngularRoadrunner services will live.
angular.module("roadrunner", []).value("Roadrunner", Roadrunner);

// Define the `angularRoadrunner` service for implicit syncing. `angularRoadrunner` binds a
// model to $scope and keeps the data synchronized with a Roadrunner location
// both ways.
angular.module("roadrunner").factory("angularRoadrunner", ["$q", "$parse", "$timeout",
  function($q, $parse, $timeout) {
    // The factory returns a new instance of the `AngularRoadrunner` object, defined
...
AngularRoadrunner.prototype = {
  ...
  // [pull request #34](https://github.com/roadrunner/angularRoadrunner/pull/34).
...
// `angularRoadrunnerCollection` provides a collection object that you can modify.
// [Original code](https://github.com/petebacondarwin/angular-roadrunner/blob/master/ng-roadrunner-collection.js)
// by @petebacondarwin.
angular.module("roadrunner").factory("angularRoadrunnerCollection", ["$timeout",
  function($timeout) {
