...
// Define the `roadrunner` module under which all AngularRoadrunner services
// will live.
angular.module("roadrunner", []).value("Roadrunner", Roadrunner);

// Define the `angularRoadrunner` service for implicit syncing.
// `angularRoadrunner` binds a
// model to $scope and keeps the data synchronized with a Roadrunner location
// both ways.
angular.module("roadrunner").factory("angularRoadrunner", [ "$q", "$parse", "$timeout", function($q, $parse, $timeout) {
	// The factory returns a new instance of the `AngularRoadrunner` object,
	// defined
	// below, everytime it is called. The factory takes 4 arguments:
	//
	// * `ref`: A Roadrunner URL or reference. A reference with limits
	...
	// (or is present on the Roadrunner URL provided). Pass in
	// `{}` for Object, `[]` for Array (default), `""` for
	// String and `true` for Boolean.
	return function(ref, scope, name, type) {
		var af = new AngularRoadrunner($q, $parse, $timeout, ref);
		return af.associate(scope, name, type);
	};
} ]);

// The `AngularRoadrunner` object that implements implicit synchronization.
function AngularRoadrunner($q, $parse, $timeout, ref) {
	this._q = $q;
	this._parse = $parse;
	...
	// `ref` can either be a string (URL to a Roadrunner location), or a
	// `Roadrunner` object.
	if (typeof ref == "string") {
		this._fRef = new Roadrunner(ref);
	} else {
		this._fRef = ref;
	}
}
AngularRoadrunner.prototype = {
	// This function is called by the factory to create a new 2-way binding
	// between a particular model in a `$scope` and a particular Roadrunner
	// location.
	associate : function($scope, name, type) {
		...
		// [Ticket #25](https://github.com/roadrunner/angularRoadrunner/issues/25).
	...
	// [pull request
	// #34](https://github.com/roadrunner/angularRoadrunner/pull/34).
	// This function is provided to the promise returned by `angularRoadrunner`
...
// Define the `angularRoadrunnerCollection` service for explicit syncing.
// `angularRoadrunnerCollection` provides a collection object that you can
// modify.
// [Original
// code](https://github.com/petebacondarwin/angular-roadrunner/blob/master/ng-roadrunner-collection.js)
// by @petebacondarwin.
angular.module("roadrunner").factory("angularRoadrunnerCollection", [ "$timeout", function($timeout) {
	return function(collectionUrlOrRef, initialCb) {
		// An internal representation of a model present in the collection.
		function angularRoadrunnerItem(ref, index) {
			this.$ref = ref.ref();
			this.$id = ref.name();
			this.$index = index;
			angular.extend(this, {
		...
		// The provided ref can either be a string (URL to a Roadrunner location)
		// or an object of type `Roadrunner`. Roadrunner objects with limits or
		// queries applies may also be provided.
		var collectionRef;
		if (typeof collectionUrlOrRef == "string") {
			collectionRef = new Roadrunner(collectionUrlOrRef);
		} else {
		...
		collectionRef.on("child_added", function(data, prevId) {
			$timeout(function() {
				...
				addChild(index, new angularRoadrunnerItem(data, index));
				updateIndexes(index);
			});
		});
		...
		// `angularRoadrunnerCollection` exposes three methods on the collection
		// returned.

		// Add an object to the remote collection. Adding an object is the
		// equivalent of calling `push()` on a Roadrunner reference.
...
angular.module("roadrunner").factory("angularRoadrunnerQuery", [ "$timeout", function($timeout) {
