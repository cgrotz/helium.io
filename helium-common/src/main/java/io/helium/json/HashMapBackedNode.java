...
public class HashMapBackedNode implements Node {
    ...
    public HashMapBackedNode(HeliumEventBuilder heliumEventBuilder) {
        this();
    ...
    static final Writer writeValue(Writer writer, Object value, int indentFactor, int indent)
        ...
        if (value == null || value.equals(null)) {
            writer.write("null");
        } else if (value instanceof HashMapBackedNode) {
    ...
    public Node getNode(String key) {
        Object object = this.get(key);
        if (object instanceof HashMapBackedNode) {
            return (Node) object;
        }
    ...
    public Node optNode(String key) {
        Object object = this.opt(key);
        return object instanceof HashMapBackedNode ? (Node) object : null;
    }
    ...
    public Object getObjectForPath(Path path) {
        ...
        if (node instanceof HashMapBackedNode) {
    ...
    public Collection<Node> getChildren() {
        ...
        while (itr.hasNext()) {
            Object key = itr.next();
            if (get((String) key) instanceof HashMapBackedNode) {
    ...
    public Node getLastLeafNode(Path path) {
        if (has(path.getFirstElement())) {
            if (path.isSimple()) {
                if (get(path.getFirstElement()) instanceof HashMapBackedNode) {
